...
const COLOR_MAP: Record<string, string> = {
  ...
  cyan: '#06b6d4', pink: '#ec4899', amber: '#f59e0b',
...
export function TemplateCard() {
  ...
  const iconName = iconTemplate && !iconTemplate.startsWith('[Erreur')
    ? resolveIconName(iconTemplate)
    : '';
  const IconComponent = iconName ? resolveIcon(iconName) : null;
  const iconColor = iconColorTemplate && !iconColorTemplate.startsWith('[Erreur')
    ? resolveColor(iconColorTemplate)
    : undefined;
  const imageUrl = imageTemplate && !imageTemplate.startsWith('[Erreur')
    ? imageTemplate
    : '';
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {!hasPrimary && !hasSecondary && (
          <div className="text-white/20 text-sm">Carte template</div>
        )}
      </div>
    </motion.div>
  );
