...
export function ImageBackgroundPicker({
  ...
  const { t } = useI18n();
  const [mode, setMode] = useState<ImageInputMode>(() => (background.imageUrl?.startsWith('/uploads/') ? 'file' : 'url'));
  const [uploading, setUploading] = useState(false);
  const [uploadError, setUploadError] = useState<string | null>(null);
  const fileInputRef = useRef<HTMLInputElement>(null);

  // Nom du fichier actuellement utilisé (pour l'affichage)
  const currentFilename = background.imageUrl?.startsWith('/uploads/') ? background.imageUrl.split('/').pop() : null;
  ...
  async function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const res = await apiFetch('/api/uploads/background', { method: 'POST', body: formData });
  ...
  async function handleDeleteUpload() {
    ...
    try {
      await apiFetch(`/api/uploads/background/${encodeURIComponent(currentFilename)}`, { method: 'DELETE' });
    } catch {
  ...
  return (
    <div className='flex flex-col gap-3'>
      ...
      <div className='flex gap-1.5 p-0.5 rounded-lg bg-white/5 border border-white/10 w-fit'>
        ...
        >
          <Link size={11} /> {t('settings.appearance_section.imageUrl')}
        </button>
        <button
        ...
        >
          <Upload size={11} /> {t('settings.appearance_section.imageFile')}
        </button>
      </div>
      ...
      {mode === 'url' && (
        <input
          type='text'
          placeholder={t('settings.appearance_section.imageUrlPlaceholder')}
      ...
      {mode === 'file' && (
        <div className='flex flex-col gap-2'>
          {currentFilename ? (
            <div className='flex items-center gap-2 px-3 py-2 rounded-lg bg-white/8 border border-white/15'>
              ...
              <button
                ...
                title={t('settings.appearance_section.imageDelete')}
              >
                <Trash2 size={13} />
              </button>
            </div>
            ...
            >
              <Upload size={13} />
              {uploading ? t('settings.appearance_section.imageUploading') : t('settings.appearance_section.imageChoose')}
            </button>
          )}
          <input
            ref={fileInputRef}
          ...
          <p className='text-white/25 text-[10px]'>{t('settings.appearance_section.imageFormats')}</p>
        </div>
      )}
    </div>
  );
